...
_TOKEN_VALUE = re.compile(r'^(?:[a-z0-9_]+|[A-Z0-9_]+)$')
...
class ObservedKey:
    ...
    uuid_like: bool = True
    uri_like: bool = True
    nested: ObservedObject | None = None
    items: ObservedKey | None = None
    # Item shapes of a split array, keyed by the item's `type`.
    variants: dict[str, ObservedObject] | None = None
...
class ObservedObject:
    ...
    samples: int = 0
    keys: dict[str, ObservedKey] = field(default_factory=dict[str, ObservedKey])
...
def observe(shape: ObservedObject, obj: dict[str, object], path: str = '') -> None:
    """Count one more object at `path` into `shape`, recursing into its values."""
    shape.samples += 1
    for key, value in obj.items():
        field_shape = shape.keys.setdefault(key, ObservedKey())
...
def _observe_value(shape: ObservedKey, value: object, path: str) -> None:
    shape.seen += 1
    shape.types[json_type(value)] += 1
    if isinstance(value, str):
        _observe_string(shape, value, path)
    elif isinstance(value, dict):
        if shape.nested is None:
            shape.nested = ObservedObject()
        observe(shape.nested, cast('dict[str, object]', value), path)
    elif isinstance(value, list):
        _observe_list(shape, cast('list[object]', value), path)


def _observe_string(shape: ObservedKey, value: str, path: str) -> None:
    ...
    if shape.vocabulary is None or not _is_vocabulary_key(path):
        return
    if _TOKEN_VALUE.match(value) is None:
...
def _is_vocabulary_key(path: str) -> bool:
    return last_key(path).lower().endswith(VOCABULARY_KEY_SUFFIXES)


def _observe_list(shape: ObservedKey, items: list[object], path: str) -> None:
    if path in ARRAYS_SPLIT_BY_TYPE:
        if shape.variants is None:
            shape.variants = {}
        for item in items:
            kind = _kind_of(item, path)
            variant = shape.variants.setdefault(kind, ObservedObject())

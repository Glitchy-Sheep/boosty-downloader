...
class TestOAuthManager(unittest.TestCase):
    ...
    def test_no_tokens_initially(self):
        ...
        self.assertFalse(self.oauth_manager.has_tokens())
        self.assertTrue(self.oauth_manager.is_expired())

    def test_set_and_get_tokens(self):
        """Test setting and getting OAuth tokens"""
        self.oauth_manager.set_tokens(self.sample_tokens)
        
        self.assertTrue(self.oauth_manager.has_tokens())
        self.assertFalse(self.oauth_manager.is_expired())
        self.assertEqual(
            self.oauth_manager.get_access_token(),
            self.sample_tokens.access_token,
        )
    ...
    def test_save_and_load_tokens(self):
        ...
        self.assertTrue(new_manager.has_tokens())
        self.assertEqual(
            new_manager.get_access_token(),
            self.sample_tokens.access_token,
        )
    ...
    def test_expired_tokens(self):
        ...
        expired_tokens = OAuthTokens(
            ...
            expires_at=int(datetime.now().timestamp()) - 3600,  # 1 hour ago
    ...
    def test_file_persistence(self):
        ...
        self.assertEqual(data['access_token'], self.sample_tokens.access_token)
        self.assertEqual(data['refresh_token'], self.sample_tokens.refresh_token)
        self.assertEqual(data['device_id'], self.sample_tokens.device_id)

...
- An external video that is gone for good (deleted, private, blocked, an unsupported link, a 404 page) gets no retries: yt-dlp marks such answers as expected, the wrapper raises `ExtVideoUnavailableError` with the site's reason, and the retrier ends the post after the first attempt. The post counts as failed, its page carries the placeholder, and the next run checks the video once more, but it does not count toward the failure streak: a deleted video says nothing about the disk or the network.

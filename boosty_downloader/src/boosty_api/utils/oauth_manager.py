...
from boosty_downloader.src.boosty_api.models.oauth import OAuthRefreshResponse, OAuthTokens
...
class OAuthManager:
    ...
    def get_access_token(self) -> str:
        ...
        if self._tokens is None:
            raise OAuthError('No OAuth tokens available')
    ...
    async def _refresh_tokens(self, session: RetryClient) -> None:
        ...
        if self._tokens is None:
            raise OAuthRefreshError('No tokens to refresh')
        ...
        try:
            async with session.post(
                ...
                if resp.status != 200:
                    raise OAuthRefreshError(f'Token refresh failed with status {resp.status}')
                ...
                self._tokens = OAuthTokens(
                    ...
                    expires_at=int(datetime.now().timestamp()) + refresh_response.expires_in,
                    device_id=self._tokens.device_id,
                )
            ...
            raise OAuthRefreshError(f'Token refresh failed: {e}') from e
